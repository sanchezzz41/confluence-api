 ...
 * @this {Confluence}
 * @param {Object} config
 * @param {string} config.username - Optional
 * @param {string} config.password - Optional
...
function Confluence(config) {
    ...
    else if ((!config.username && !config.password) && !config) {
        throw new Error("Confluence module expects a config object with both a username and password or PAT.");
    }
...
Confluence.prototype.getSpace = function(space, callback){
    ...
    request
        ...
        .set('Authorization', `${auth(this.config)}`)
